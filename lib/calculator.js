 ...
 * 남은 근무 정보 계산
 * @param {object} params
 * @param {number} params.accumulatedMinutes - 누적 근무시간 (분 단위, 페이지에서 휴가 크레딧 포함됨)
 * @param {number} params.requiredHours - 의무 근로시간 (시간 단위, 휴가일도 워크데이로 카운트)
 * @param {number|null} params.pageRemainingDays - 페이지에서 읽은 남은 근무일
 ...
 * @param {string[]} [params.vacationDates] - 사용자 지정 휴가 날짜 목록 ("YYYY-MM-DD")
...
function calcRemainingInfo({
  ...
  // 오늘 이후(포함) 남은 근무일 — 공휴일 & 휴가 제외
  const calcWorkdays = weekdays.filter(
    (d) => d >= today && !holidaySet.has(d) && !vacationSet.has(d)
  );

  // 오늘 이후 휴가 일수 (페이지 남은 일수 보정용)
  const futureVacationCount = vacationDates.filter((d) => d >= today).length;

  // 남은 근무일: 페이지 값이 있으면 휴가 제외 후 사용, 없으면 달력 계산
  const totalRemainingDays =
    pageRemainingDays != null
      ? Math.max(0, pageRemainingDays - futureVacationCount)
      : calcWorkdays.length;
